...
public class DBUtil {
    ...
    public static List<Map<String, Object>> queryMapListForEncode(Connection con, String sql, Object... params)
            throws JdbcException {
            ...
            try {
                ...
                while (null != rs && rs.next()) {
                    ...
                    for (int i = 0; i < columnCount; i++) {
                        String name = rsmd.getColumnName(i + 1);
                        Object value = rs.getObject(name);
                        if (null != value && StringUtils.contains(value.toString(), "<")) {
                            value = new String(Base64.encodeBase64(value.toString().getBytes()), "UTF-8");
                        }
                        map.put(name, value);
                    }
                    lists.add(map);
